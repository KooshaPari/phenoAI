...
class ResearchStore:
    ...
    def get_recent(self, *, hours: int = 24, limit: int = 50) -> list[ResearchItem]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM research_items WHERE fetched_at >= ? "
                "ORDER BY relevance DESC, score DESC LIMIT ?",
                (cutoff, limit),
            ).fetchall()
    ...
    def mirror_to_project(
        self, project_db: Path, *, min_relevance: float = 0.3
    ) -> int:
        ...
        Args:
            ...
            min_relevance: Only copy items with relevance >= this value.
    ...
    def _get_by_relevance(self, min_relevance: float) -> list[ResearchItem]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM research_items WHERE relevance >= ? "
